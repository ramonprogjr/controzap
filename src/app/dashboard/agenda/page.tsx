...
const STATUS_CONFIG = {
  ...
  confirmed: { label: 'Confirmado', color: 'bg-green-500/10 text-green-400 border-green-500/20',      icon: CheckCircle },
  completed: { label: 'Concluído',  color: 'bg-green-500/10 text-green-400 border-green-500/20',   icon: CheckCircle },
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-8">
        ...
        <div className="bg-card-theme border border-main rounded-3xl p-6 h-fit shadow-xl">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSel ? "bg-green-600 text-white shadow-lg shadow-green-500/20" :
                    isTod ? "bg-green-500/20 text-green-400 ring-2 ring-green-500/30" :
          ...
          <div className="flex items-center gap-4 mt-6 pt-4 border-t border-main">
            ...
            <div className="flex items-center gap-2 text-xs text-dim font-bold">
              <span className="w-4 h-4 rounded-lg bg-green-500/20 ring-2 ring-green-500/30 inline-block" />
              Hoje
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredAppointments.map(appt => {
                ...
                return (
                  <div key={appt.id} className="bg-card-theme border border-main rounded-2xl p-6 hover:border-green-500/30 transition-all shadow-lg group">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-4 flex-1 min-w-0">
                        <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 rounded-xl flex items-center justify-center shadow-lg shadow-green-500/20 shrink-0">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <select
                          ...
                          className="bg-slate-800 border border-slate-700 text-white text-xs font-bold rounded-xl px-3 py-2 cursor-pointer focus:outline-none focus:ring-2 focus:ring-green-500/50"
                        >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.clientName} onChange={e => setForm(f => ({ ...f, clientName: e.target.value }))}
                    placeholder="Ex: João Silva" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 font-medium" />
                </div>
                <div>
                  ...
                  <input type="text" value={form.clientPhone} onChange={e => setForm(f => ({ ...f, clientPhone: e.target.value }))}
                    placeholder="11999999999" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 font-medium" />
                </div>
                <div>
                  ...
                  <select value={form.seller_id} onChange={e => setForm(f => ({ ...f, seller_id: e.target.value }))}
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-green-500/50 font-bold cursor-pointer">
                ...
                <div>
                  ...
                  <input type="date" value={form.detected_date} onChange={e => setForm(f => ({ ...f, detected_date: e.target.value }))}
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-green-500/50 font-medium" />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                    placeholder="Ex: Aeroporto de Congonhas" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 font-medium" />
                </div>
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                    placeholder="Ex: Toyota Corolla - 3 dias" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 font-medium" />
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreate} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white px-4 py-3 rounded-xl font-black text-sm uppercase tracking-wider transition-all shadow-lg shadow-green-500/20 disabled:opacity-50">
